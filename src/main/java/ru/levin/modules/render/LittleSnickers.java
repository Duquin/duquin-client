...
import ru.levin.modules.render.littlePet.GhostBeeEntity;
import ru.levin.modules.render.littlePet.GhostCatEntity;
import ru.levin.modules.render.littlePet.GhostParrotEntity;
...
@FunctionAnnotation(name = "LittlePet", desc = "Питомец следует за вами: пёс, кот, пчела или попугай на плече", type = Type.Render)
public class LittleSnickers extends Function {
    ...
    private final ModeSetting pet = new ModeSetting("Питомец", "Собака", "Собака", "Кошка", "Пчела", "Попугай");
    ...
    public LittleSnickers() {
        addSettings(pet, bumbum);
    }

    private boolean isFlying() {
        return pet.is("Пчела") || pet.is("Попугай");
    }

    private LivingEntity createEntity() {
        LivingEntity entity = switch (pet.get()) {
            case "Кошка" -> new GhostCatEntity(EntityType.CAT, mc.world);
            case "Пчела" -> new GhostBeeEntity(EntityType.BEE, mc.world);
            case "Попугай" -> new GhostParrotEntity(EntityType.PARROT, mc.world);
            default -> new GhostWolfEntity(EntityType.WOLF, mc.world);
        };
    ...
    private void moveFlyingTowards(Vec3d target) {
        Vec3d entityPos = fakeEntity.getPos();
        Vec3d delta = target.subtract(entityPos);
        double d = delta.length();

        if (d > 6) {
            fakeEntity.refreshPositionAndAngles(target.x, target.y, target.z, fakeEntity.getYaw(), fakeEntity.getPitch());
            fakeEntity.setVelocity(Vec3d.ZERO);
            return;
        }

        if (d > 0.05) {
            double step = Math.min(d, 0.45);
            fakeEntity.move(MovementType.SELF, new Vec3d(delta.x / d * step, delta.y / d * step, delta.z / d * step));
        }
        fakeEntity.setVelocity(Vec3d.ZERO);
    ...
    private Vec3d computeAnchor() {
        ...
        if (pet.is("Попугай")) {
            ...
            double side = -0.32;
            return new Vec3d(
                    mc.player.getX() + lx * side,
                    mc.player.getY() + 1.32,
                    mc.player.getZ() + lz * side
            );
        }

        double bob = Math.sin(System.currentTimeMillis() / 400.0) * 0.08;
        return new Vec3d(
                mc.player.getX() - fx * 1.1,
                mc.player.getY() + 0.75 + bob,
                mc.player.getZ() - fz * 1.1
        );
    }
    ...
    private void moveEntityTowardsTarget(Vec3d targetPos) {
        ...
        fakeEntity.setSprinting(isMoving && !isJumping);
        if (fakeEntity instanceof TameableEntity tameable) {
            tameable.setSitting(!isMoving && onGround);
        }
    }
    ...
    private final ClientTickEvents.EndTick tickListener = client -> {
        ...
        if (isFlying()) {
            moveFlyingTowards(computeAnchor());
            return;
        }
